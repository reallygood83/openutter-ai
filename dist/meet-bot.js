...
async function isBlockedFromJoining(page) {
    try {
...
async function dismissPostJoinDialogs(page) {
...
async function disableMediaOnPreJoin(page, opts) {
    if (opts.noMic) {
        try {
            const micBtn = page
                .locator('[aria-label*="microphone" i][data-is-muted="false"], ' +
    ...
    if (opts.noCamera) {
        try {
            const camBtn = page
                .locator('[aria-label*="camera" i][data-is-muted="false"], ' +
...
async function enterNameIfNeeded(page, botName) {
    try {
...
async function clickJoinButton(page, maxAttempts = 6) {
    const joinSelectors = [
...
async function waitUntilInMeeting(page, timeoutMs = 600_000) {
...
async function clickLeaveButton(page) {
    try {
        const leaveBtn = page
            .locator('[aria-label*="Leave call" i], [aria-label*="leave" i][data-tooltip*="Leave"]')
            .first();
...
async function waitForMeetingEnd(page, opts) {
    const start = Date.now();
...
async function enableCaptions(page) {
    ...
    const checkCaptions = async () => page
        .evaluate(`!!(document.querySelector('[role="region"][aria-label*="Captions"]') ||
    ...
    try {
        ...
        await page.waitForTimeout(1000);
        const ccButton = page
            .locator('button[aria-label*="Turn on captions" i], ' +
...
function extractMeetingId(meetUrl) {
    try {
...
function registerScreenshotHandler(page) {
...
export async function runBot(config) {
    const { meetUrl, headed, anon: noAuth, camera, mic, verbose, durationMs, botName: botNameOpt, channel, target, ai: aiEnabled, aiModel, aiSystemPrompt, aiVoice, aiTrigger, openaiApiKey, } = config;
    ...
    const onCaption = aiEnabled
        ? (speaker, text) => {
            if (!aiResponder || !ttsEngine)
                return;
            // Skip captions from the bot itself
            if (speaker.toLowerCase() === botName.toLowerCase())
                return;
            ...
            aiProcessingQueue = aiProcessingQueue.then(async () => {
                try {
                    ...
                    console.error("  [ai] Pipeline error:", err instanceof Error ? err.message : String(err));
                }
            });
        }
        : undefined;
    // Start caption capture
    const { cleanup: cleanupCaptions, getLastCaptionAt } = await setupCaptionCapture(currentPage, transcriptPath, verbose, onCaption ? { onCaption } : undefined);
...
//# sourceMappingURL=meet-bot.js.map
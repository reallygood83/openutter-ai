 ...
 */
export async function setupCaptionCapture(page, transcriptPath, verbose, options) {
    ...
    const finalizeCaption = (speaker, text, startTs) => {
        ...
        if (onCaptionCallback) {
            try {
                ...
                console.error("  [caption] onCaption callback error:", err instanceof Error ? err.message : String(err));
            }
        }
    };
    // Bridge browser -> Node.js
    await page.exposeFunction("__openutter_onCaption", (speaker, text) => {
        ...
        if (existing) {
            ...
            const isGrowing = normNew.startsWith(normOld) ||
...
//# sourceMappingURL=caption-capture.js.map
 ...
 */
export async function setupVirtualAudio(context: BrowserContext): Promise<void> {
 ...
 */
export async function injectAudio(page: Page, audioBuffer: Buffer): Promise<void> {
  const base64Audio = audioBuffer.toString("base64");

  const success = await page.evaluate(async (b64: string) => {
    const audioState = (window as any).__openutter_audio;
    if (!audioState || !audioState.ctx || !audioState.dest) {
      console.error("[OpenUtter] Virtual audio not initialized");
      return false;
    }

    const { ctx, dest } = audioState as {
      ctx: AudioContext;
